...
async function togglePlayback() {
  try {
    ...
    } else if (error.name === 'NotSupportedError' || video.error?.code === 4) {
      message = 'No se encontró el video o su formato no es compatible.';
    } else if (video.error?.code === 2) {
      message = 'Ocurrió un error de red al cargar el video.';
    } else if (video.error?.code === 3) {
      message = 'El navegador no pudo decodificar el video.';
    }
...
function schedulePause() {
  ...
  // Se calcula desde la posición actual del video.
...
function checkScheduledPause() {
  ...
  if (video.currentTime >= scheduledPauseTime) {
    ...
    setStatus('El temporizador terminó y el video fue pausado.', 'active');
  }
}
...
player.addEventListener('mouseleave', scheduleControlsHide);
